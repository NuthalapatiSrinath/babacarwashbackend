...
controller.signup = async (req, res) => {
    try {
        ...
        if (error == "ALREADY-REGISTERED") {
            return res.status(401).json({ status: false, message: "The email is already regsitered. Please sign in." })
        }
...
controller.verification = async (req, res) => {
    try {
        const { body } = req
        const data = await service.verification(body)
...
controller.verificationValidation = async (req, res) => {
    try {
        const { body } = req
        const data = await service.verificationValidation(body)
        return res.status(200).json({ statusCode: 200, message: 'success', data })
    } catch (error) {
        console.error(error)

        if (error == "INVALID") {
            return res.status(401).json({ status: false, message: "OTP is invalid. Please enter correct OTP" })
        }
