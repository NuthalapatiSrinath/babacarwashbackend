...
service.update = async (userInfo, id, payload) => {
  ...
  const isExists = await PricingsModel.countDocuments({
    _id: { $ne: id },
    ...findQuery,
  });
  if (isExists) {
...
service.delete = async (userInfo, id, payload) => {
  return await PricingsModel.updateOne(
    { _id: id },
...
service.getByMall = async (userInfo, mallId) => {
  return await PricingsModel.findOne({ mall: mallId, isDeleted: false }).lean();
};
