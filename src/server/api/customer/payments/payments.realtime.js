...
const sendToCustomer = (customerId, payload) => {
  ...
  for (const client of [...clients]) {
    const ok = sendEvent(client.res, "payment-change", payload);
    if (!ok) {
      try {
        client.cleanup();
      } catch (_) {}
...
const fetchLatestPaymentVersion = async (customerId) => {
  const latest = await PaymentsModel.findOne({
    isDeleted: false,
...
const runPollCycle = async () => {
  ...
  for (const customerId of customerIds) {
    try {
      const latest = await fetchLatestPaymentVersion(customerId);
      if (!latest) continue;
...
const bootstrapWatcher = () => {
  ...
  try {
    ...
    watcher.on("change", (change) => {
      ...
      const payload = {
        operation,
        paymentId: String(change?.documentKey?._id || ""),
        customer: customerId,
      };
    ...
    watcher.on("error", (error) => {
      ...
      console.error("[SSE] Payments watcher error:", error?.message || error);
      watcherBootstrapped = false;
      watcher = null;
    ...
    console.error(
      "[SSE] Failed to start payments watcher:",
      error?.message || error,
    );
    watcherBootstrapped = false;
    watcher = null;
