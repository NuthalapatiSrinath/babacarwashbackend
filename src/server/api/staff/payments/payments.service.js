...
service.list = async (userInfo, query) => {
    ...
    const findQuery = {
        ...
        ...(query.search ? {
            $or: [
                { "vehicle.registration_no": { $regex: new RegExp(query.search.trim(), 'i') } },
                { "vehicle.parking_no": { $regex: new RegExp(query.search.trim(), 'i') } }
            ]
        } : null),
        ...(query.status ? {
            status: query.status,
    ...
    const completedAmount = await PaymentsModel.find({ ...findQuery, status: 'completed', settled: 'pending' }, { amount_paid: 1 }).lean()
    ...
    const counts = {
        ...
        pendingAmount: pending.reduce((p, c) => p + (c.amount_charged + c.old_balance), 0),
...
service.collectOnewashPayment = async (userInfo, id, payload, paymentData) => {
    ...
    if (jobData.mall) {
        ...
        if (payload.payment_mode != 'cash') {
            ...
            tip_amount = payload.amount > finalAmount ? payload.amount - finalAmount : 0
        }
    }
    ...
    await OneWashModel.updateOne({ _id: jobData._id }, {
        $set: {
            tip_amount,
            amount: amount_paid,
            payment_mode: payload.payment_mode,
...
service.collectPayment = async (userInfo, id, payload) => {
    ...
    await PaymentsModel.updateOne({ _id: id }, {
        ...
        $set: { payment_mode: payload.payment_mode, balance, status, collectedDate: new Date() }
    })
